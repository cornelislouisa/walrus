...
# Usage:
#   bash run_scripts/morphogenesis_mit_finetune.sh
# Optional:
#   CUDA_VISIBLE_DEVICES=1 NGPUS=2 bash run_scripts/morphogenesis_mit_finetune.sh
...
torchrun \
  ...
  name=Walrus_ft_morphogenesis_mit \
  trainer=globalnorm \
  trainer.grad_acc_steps=1 \
  optimizer=adam \
  optimizer.lr=1.e-4 \
  logger.wandb_project_name="morphogenesis_mit" \
