...
# Constant mean-field baseline configuration on morphogenesis WT.
#
...
# The zero-shot analysis path computes a frozen mean from the run's original
# training data before evaluating a target dataset.
#
...
#     ++folder_override=/path/to/runs/MeanField_morph_WT_eval
# Optional:
#   CUDA_VISIBLE_DEVICES=1 NGPUS=1 bash run_scripts/baselines/mean_field_morphogenesis.sh
...
echo "Mean-field baseline on morphogenesis WT"
