...
# Train SineNet from scratch on the same morphogenesis WT data used by
# morphogenesis_finetune.sh (velocity-only, no myosin).
#
# SineNet is natively 2D, so the loader is told to keep the data 2D
...
# Optional:
#   CUDA_VISIBLE_DEVICES=1 NGPUS=1 bash run_scripts/baselines/sinenet_morphogenesis.sh
#   EXPERIMENT_DIR=/path/to/runs bash run_scripts/baselines/sinenet_morphogenesis.sh
...
echo "SineNet baseline on morphogenesis WT"
